...
namespace IT
{
	...
	void IdTable::PrintIdTable(const wchar_t* in)
	{
		...
		if (idStream->is_open())
		{
			...
			*(idStream) << std::setw(15) << "Идентификатор:" << std::setw(17) << "Тип данных:" << std::setw(15) << "Значение:" << std::setw(35) << "Кол-во переданных параметров:" << std::setw(20) << "Первое вхождение:" << std::endl;
			...
			for (int i = 0; i < this->size; i++)
			{
				bool flagAgain = false;
					...
					case IT::IDTYPE::P:
						...
						{
						case 1:
							*(idStream) << "   " << this->table[i].id << std::setw(24) << "INT " << "\t\t" << 0 << std::setw(15) << "\t\t\t\t\t" << this->table[i].firstApi << std::endl;
							break;
						case 2:
							*(idStream) << "   " << this->table[i].id << std::setw(23) << "STR " << "\t    " << "   \"\"" << " \t\t\t\t\t\t" << this->table[i].firstApi << std::endl;
							break;
						}
						break;
					...
					case IT::IDTYPE::L:
						...
						case 1:
							...
							if (!flagAgain)
							{
								*(idStream) << "   " << tempBuf << "\t\t\t" << "INT " << "\t\t" << this->table[i].value.vint << " \t\t\t\t\t\t" << this->table[i].firstApi << std::endl;
						...
						case 2:
							...
							if (!flagAgain)
							{
								*(idStream) << "   " << tempBuf << "\t\t\t" << "STR " << "\t    " << this->table[i].value.vstr.str << " \t\t\t\t\t\t" << this->table[i].firstApi << std::endl;
					...
					case IT::IDTYPE::F:
						...
						{
						case 1:
							*(idStream) << "   " << this->table[i].id << std::setw(25 - strlen(this->table[i].id)) << "INT " << "\t\t\t\t\t" << this->table[i].parmQuantity << "\t\t\t" << this->table[i].firstApi << std::endl;
							break;
						case 2:
							*(idStream) << "   " << this->table[i].id << std::setw(25 - strlen(this->table[i].id)) << "STR " << "\t\t\t\t\t" << this->table[i].parmQuantity << "\t\t\t" << this->table[i].firstApi << std::endl;
							break;
						}
						break;
					}
